...
| [Signals Page — Live Data + Badge Rethink](changes/signals-page-live-data/OPENSPEC.md) | 2026-04-17 | _pending commit_ | **Fix: Signals page showed stale/contradictory data** (e.g. MSFT "EXIT SIGNAL + BULL + Bear Flag" at $411.22). Removed hardcoded 13-entry `TICKERS` literal (~160 lines of mock data incl. fake sparklines, fake pattern/analysis strings, fake stop/entry/T1/T2 levels). Signals page now renders **entirely from `data/watchlist.json`** via new `mapWatchlistToSignal()` mapper. Canonical status enum (`exit \| triggered \| approaching \| active \| watching`) wired end-to-end. Replaced fake sparkline with **Technical Range bar** (52w low → 52w high + SMA20/50/200 markers, hover tooltips). Every badge + level pill + range marker now has a `title=` tooltip explaining the current value and why it's lit. Dropped redundant pattern + volatility badges (ATR% shown in stats row instead). Added RSI overbought/oversold + earnings-proximity contextual badges. Tests: `tests/signals.test.js` — 13 Jest tests incl. live-data integration guard against contradiction bugs. Docs: `docs/SIGNALS.md`. |
| [EM Page — Sticky Header Fix (SPY Hidden)](changes/em-sticky-header-fix/OPENSPEC.md) | 2026-04-17 | _pending commit_ | **Fix: SPY (first data row) visually hidden behind the sticky table header** on `v1/expected-moves.html`. Root cause: inline `<style>` set `thead th { top: 0 }`, pinning the sticky header to viewport y=0 underneath the 48px top nav. Updated to `top: var(--nav-height, 48px)` + added `box-shadow` separation + `tbody tr:first-child td { padding-top: 12px }` for breathing room. Same polish applied to `css/expected-moves.css` (v2 SPA) for consistency. CSS-only change, no JS. |
