...
1. **Static HTML vs Next.js?** Static ships faster. Next.js scales better with Tom agent API routes. Recommend: **Start static, migrate later.**
2. **Tom agent hosting?** Azure OpenAI (existing resource) vs Anthropic API vs local? Need streaming for good chat UX.
3. **Auth?** Public dashboard or private? If private, need auth layer.
4. **Real-time data?** Current pipeline is batch (daily). WebSocket for live prices would be nice but adds complexity.
5. **Historical setups?** Track win/loss on past setups for track record? Would need a DB.
6. **Alerts/Notifications?** Push to Telegram when new setups trigger? (We already have the Telegram infra from BreakingTrades bot)
